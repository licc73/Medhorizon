...
class MyMessageViewController: UIViewController {
    ...
    func getMessageFromServer(page: Int) {
        ...
        DefaultServiceRequests.rac_requesForMyMessageList(userId, pageNum: page, pageSize: pageSize)
                ...
                next: {[unowned self] (page, data) in
                    ...
                    if let msg = returnMsg where msg.isSuccess {
                        ...
                        if let messageList = data["InfoList"] as? [[String: AnyObject]] {
                            let myMessage = messageList.flatMap {MyMessageViewModel.mapToModel($0)}
                            if serverFirstPageNum == page {
                                self.messageList = myMessage
                            }
                            else {
                                self.messageList.appendContentsOf(myMessage)
                            }
...
extension MyMessageViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.myMessage = self.messageList[indexPath.row]
        return cell
    }
